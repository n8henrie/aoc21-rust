...
fn multiply(fish: &[u8], days: u32) -> Vec<u8> {
    ...
    for _ in 0..days {
        ...
        fish = fish
            .iter()
            .map(|f| {
                f.checked_sub(1).unwrap_or_else(|| {
                    extras += 1;
                    6
                })
            })
            .collect();
        fish.append(&mut vec![8; extras]);
    }
    fish.to_vec()
}

fn part1(fish: &[u8], days: u32) -> usize {
    multiply(fish, days).len()
}
...
fn main() -> anyhow::Result<()> {
    ...
    println!("day 06 part 1: {}", part1(&mut fish, 80));
...
mod tests {
    ...
    fn test_multiply() {
        let fish = parse_input(EXAMPLE_INPUT).unwrap();
        let result = multiply(&fish, 5);
        assert_eq!(result, vec![5, 6, 5, 3, 4, 5, 6, 7, 7, 8]);
    }
    ...
    #[test]
    fn test_part2() {
        let mut fish = parse_input(EXAMPLE_INPUT).unwrap();
        assert_eq!(part1(&mut fish, 256), 26984457539);
    }
}
